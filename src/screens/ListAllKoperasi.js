...
export default function ListAllKoperasi(props) {
    ...
    const btnDelete = useCallback((value) => {
        HttpRequest.deletedKoperasi(value).then((res) => {
    ...
    return (
        <>
            <SafeAreaView style={styles.container}>
                ...
                >
                    <Text style={styles.txtHeader}>Daftar List</Text>
                </HeaderBack>
                <View style={{ padding: 20, flex: 1 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Text style={[styles.txtBoldGlobal, { fontSize: 17, color: color.black, flex: 1 }]}>Daftar list</Text>
                        <TouchableOpacity activeOpacity={1} onPress={() => {
                            navigation.navigate("TambahKoperasiList")
                                    ...
                                    <>
                                        <View style={[{ flexDirection: 'column', backgroundColor: color.white, padding: 18, borderRadius: 8 }]}>
                                            <View style={{ flexDirection: 'row' }}>
                                                <Text style={[styles.txtBoldGlobal, { color: color.black, flex: 1 }]}>{item.nama}</Text>
                                            </View>
                                            <View style={{ flexDirection: 'row', marginVertical: 8 }}>
                                                <View style={{ flexDirection: "row", flex: 1 }}>
                                                    <Text style={[styles.txtGlobal]}>Nominal : </Text>
                                                    <Text style={[styles.txtBoldGlobal, { fontSize: 14, color: color.black }]}>{Rupiah.format(item.harga)}</Text>
                                                </View>
                                                {/* <View style={{ flexDirection: "row", flex: 1 }}> */}
                                            ...
                                            <View style={{ flexDirection: 'row' }}>
                                                <TouchableOpacity activeOpacity={1} onPress={() => {
                                                    navigation.navigate("EditKoperasiList", { params: item })
