...
export default function ListTotalKehadiran(props) {
    ...
    const loadListKeterangan = useCallback(() => {
        HttpRequest.listAbsenByUserID(user.data.user_id).then((res) => {
    ...
    return (
        <>
            <SafeAreaView style={styles.container}>
                ...
                >
                    <Text style={styles.txtHeader}>List Absen anak</Text>
                </HeaderBack>
                <View style={{ padding: 20, flex: 1 }}>
                    {/* <View style={{ backgroundColor: color.white, padding: 20, borderRadius: 12 }}> */}
                    {/* <View style={{ flexDirection: 'row' }}>
                            <Text style={[styles.txtGlobalBold, { color: color.black, flex: 1 }]}>Total Kehadiran Anak</Text>
                            ...
                            listAbsen.map((item, iList) => {
                                return (
                                    <>
                                        ...
                                        }} style={{ flexDirection: 'column', backgroundColor: color.white, padding: 18, borderRadius: 12 }}>
                                            <Text style={[styles.txtGlobalBold, { fontSize: 12, color: color.black, marginVertical: 6, textAlign: 'left' }]}>Nama : {item.nama_lengkap}</Text>
